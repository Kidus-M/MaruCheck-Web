        ...
        {contracts.map((contract) => (
          <article className="contract-card" key={contract.id}>
            ...
            <div className="contract-card__statement">
              ...
              <p>
                {contract.id === "invoice-access"
                  ? "Every invoice read is authorized against the authenticated account."
                  : contract.id === "subscription-management"
                    ? "Subscription state changes only after verified billing events."
                    : contract.id === "workspace-membership"
                      ? "Membership changes preserve accountable ownership and least privilege."
                      : "The hosted surface remains healthy, attributable, and observable."}
              </p>
            </div>
            <footer>
              <div>
